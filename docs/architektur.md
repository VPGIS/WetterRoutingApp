...
Die Anwendung berechnet Routen auf Basis von Straßen- bzw. Wegenetzen. Wetterinformationen können in die Bewertung der Route einbezogen werden, damit Strecken mit ungünstigen Wetterbedingungen höher gewichtet oder vermieden werden.
...
Das Frontend wird lokal im Browser geöffnet und kommuniziert über HTTP-Anfragen mit dem Backend.
...
Das Backend wird lokal gestartet. Weitere Informationen dazu befinden sich in:
...
Über die API `WAPapi` wird eine Routinganfrage vom Frontend an das Backend übergeben.
...
Im Backend wird die Anfrage validiert und an die Routing-Logik weitergegeben. Dort werden die benötigten Graph- und Wetterdaten geladen, die Route berechnet und anschließend als Antwort an das Frontend zurückgegeben.
...
1. **API-Vorbereitung:**  
   `backend/api.py` validiert die Anfrage, verarbeitet Start- und Zielpunkt, konvertiert die Geschwindigkeit und öffnet die passende NetCDF-Wetterdatei.

2. **Graph-Vorbereitung:**  
   Aus Start und Ziel wird eine Bounding Box berechnet. Anschließend wird ein passender Graph aus dem Cache geladen oder über OSMnx neu erstellt.

3. **Node-Zuordnung:**  
   Start- und Zielkoordinaten werden den nächstgelegenen Nodes im Graphen zugeordnet.

4. **Modellauswahl:**  
   Abhängig vom gewählten Routingmodell wird entweder ein einfaches oder ein zeitabhängiges Routingverfahren verwendet.

5. **Rückgabe:**  
    Nach der Routenberechnung wird die Route in ein GeoJSON-ähnliches Format umgewandelt und an das Frontend zurückgegeben.

   Weitere Details:
   <details>
   <summary><strong>📊 Ablaufdiagramm anzeigen / ausblenden</strong></summary>

![Ablaufdiagramm Verarbeitung API](grafic/Ablaufdiagramm_Verarbeitung_API.drawio.svg)
...
Weitere Details zur Routinglogik, den Algorithmen und der Kostenberechnung befinden sich in:
...
Für die Zuordnung von OSM-Kanten zum Wetterraster wird bevorzugt folgende die Datei `NC_for_Cellid.nc` verwendet. Falls diese nicht existiert wird diese beim Aufstarten des Servers erzeugt siehe: [Schritt 2: NC_for_Cellid vorbereiten](startup.md#Schritt-2:-NC_for_Cellid-vorbereiten)

Weitere Details zur Erzeugung, Auswahl und Verwendung der Wetterdaten befinden sich in:  
[weather-data.md](docs/weather-data.md)
